...
const Dashboard = () => {
  ...
  const fetchDashboardData = async (fromDate = "", toDate = "") => {
    try {
      ...
      const url = `https://localhost:7197/api/Dashboard/summary?FromDate=${fromDate}&ToDate=${toDate}`;
  ...
  // THE TRIGGER (Run once when page opens)
  useEffect(() => {
    fetchDashboardData();
  }, []);
  ...
  return (
    <div className="space-y-6 pb-8">
      <div className="flex justify-between items-center">
        <div>
          <h1
            className={`text-2xl font-bold ${dark ? "text-slate-100" : "text-gray-800"}`}
          >
            Dashboard
          </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div
          className={`p-6 rounded-xl border ${dark ? "bg-slate-900 border-slate-700 text-white" : "bg-white border-gray-200"} shadow-sm`}
        >
          <div className="flex items-center justify-between mb-4">
      ...
      <div className="bg-gray-50 dark:bg-slate-800 p-12 rounded-2xl border-2 border-dashed border-gray-200 dark:border-slate-700 text-center">
        ...
        <p className="text-sm text-gray-500">Reports API here.</p>
      </div>
    </div>
  );
};
