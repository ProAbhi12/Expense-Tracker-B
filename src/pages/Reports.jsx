...
const Reports = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [pieRes, lineRes, comparisonRes] = await Promise.all([
          fetch(`${API_BASE_URL}/pie-chart${separator}${queryString}`, { signal: controller.signal }),
          fetch(`${API_BASE_URL}/line-graph${separator}${queryString}`, { signal: controller.signal }),
          fetch(`${API_BASE_URL}/income-expense-comparison${separator}${queryString}`, { signal: controller.signal }),
        ]);

        if (!pieRes.ok) throw new Error(`Pie chart failed: ${pieRes.status}`);
        if (!lineRes.ok) throw new Error(`Line graph failed: ${lineRes.status}`);
        if (!comparisonRes.ok) throw new Error(`Comparison failed: ${comparisonRes.status}`);
        ...
        if (err.name === 'AbortError') {
          setError("API request timed out (10s). Check if backend is responding.");
        } else {
  ...
  const { totalIncome, totalExpenses, netBalance } = useMemo(() => {
    const income = comparisonData.reduce((sum, item) => sum + (item.income || 0), 0);
    const expenses = comparisonData.reduce((sum, item) => sum + (item.expense || 0), 0);
    return {
      totalIncome: income,
  ...
  return (
    <div className={`min-h-screen p-4 sm:p-6 lg:p-8 transition-colors ${dark ? "bg-slate-900" : "bg-gradient-to-br from-blue-50 via-white to-purple-50"}`}>     
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Date Range Filter */}
        <div className={`rounded-2xl shadow-lg p-4 border transition-colors ${dark ? "bg-slate-800 border-slate-700" : "bg-white border-blue-100"}`}>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
            <div>
              <label className={`block text-sm font-semibold mb-2 ${dark ? "text-slate-300" : "text-gray-700"}`}>
                From Date (Optional)
              </label>
              <input
                type="date"
          ...
          {(dateRange.fromDate || dateRange.toDate) && (
            <div className={`mt-3 text-sm ${dark ? "text-slate-400" : "text-gray-600"}`}>
              Showing data from{" "}
        ...
        {/* Error Message */}
        {error && (
          <div className={`border rounded-2xl p-4 ${dark ? "bg-red-900/30 border-red-700 text-red-300" : "bg-red-50 border-red-200 text-red-700"}`}>
            <p className="font-semibold">Error: {error}</p>
            <p className="text-sm">Make sure the backend API is running on http://localhost:5000</p>
          </div>
        )}
        ...
        {loading && (
          <div className={`text-center py-12 rounded-2xl shadow-md border ${dark ? "bg-slate-800 border-slate-700" : "bg-white border-gray-200"}`}>
            ...
            <p className={dark ? "text-slate-400" : "text-gray-600"}>Loading reports...</p>
          </div>
        )}
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className={`group relative rounded-2xl p-6 transition-all duration-300 hover:shadow-xl border ${dark ? "bg-slate-800 border-slate-700 shadow-none" : "bg-gradient-to-br from-cyan-50 to-blue-50 border-cyan-200"}`}>
                  ...
                  <p className={`text-xs ${dark ? "text-cyan-400/60" : "text-cyan-700"} opacity-0 group-hover:opacity-100 transition-opacity duration-300`}>
                ...
                <div className={`group relative rounded-2xl p-6 transition-all duration-300 hover:shadow-xl border ${dark ? "bg-slate-800 border-slate-700 shadow-none" : "bg-gradient-to-br from-rose-50 to-pink-50 border-rose-200"}`}>
                  ...
                  <p className={`text-xs ${dark ? "text-rose-400/60" : "text-rose-700"} opacity-0 group-hover:opacity-100 transition-opacity duration-300`}>
                ...
                >
                  ...
                  <p className={`text-xs opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${netBalance >= 0 ? "text-emerald-700" : "text-amber-700"}`}>
                      ...
                      {pieChartData.map((item, idx) => (
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <span className={`${dark ? "text-slate-300" : "text-gray-700"} font-medium truncate`}>{item.categoryName}</span>
                          </div>
                          <span
                            className="font-bold whitespace-nowrap"
                            style={{ color: item.color || CATEGORY_COLORS[idx % CATEGORY_COLORS.length] }}
                          >
                      ...
                      <PieChart margin={{ top: 10, right: 10, left: 10, bottom: 10 }}>
                        ...
                        >
                          {pieChartData.map((_, idx) => (
                            <Cell key={`cell-${idx}`} fill={pieChartData[idx]?.color || CATEGORY_COLORS[idx % CATEGORY_COLORS.length]} />    
                          ))}
                        </Pie>
                        <Tooltip content={<CustomPieTooltip />} />
                      </PieChart>
